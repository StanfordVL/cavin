...
class TFRecordReplayBuffer(tfrecord_replay_buffer.TFRecordReplayBuffer):
    ...
    def _as_dataset(self,  # NOQA
        ...
        # data_tf.nest.flatten does not flatten python lists,
        # tf.nest.flatten does.
        flat_data_spec = tf.nest.flatten(self._data_spec)
        if flat_data_spec != data_nest.flatten(self._data_spec):
            raise ValueError(
                'Cannot perform gather; data spec contains lists and this '
                'conflicts with gathering operator. Convert any lists to '
                'tuples. For example, if your spec looks like [a, b, c], '
                'change it to (a, b, c). Spec structure is:\n {}'.format(
                    tf.nest.map_structure(
                        lambda spec: spec.dtype, self._data_spec)))

        filename_seed = distributions_util.gen_new_seed(
            self._data.seed, salt='filename_seed')

        batch_seed = distributions_util.gen_new_seed(
            self._data.seed, salt='batch_seed')

        drop_block_seed = distributions_util.gen_new_seed(
            self._data.seed, salt='drop_block')
        ...
        dropping_blocks = (tf.is_tensor(block_keep_prob) or
                           block_keep_prob != 1.0)
        if dropping_blocks:
            ...
            def select_true_or_empty(_):
                # When this returns 0, select the true block.
                # When this returns 1, select the empty block.
                return tf.cast(
                    (tf.random.uniform((), seed=drop_block_seed) >
                     block_keep_prob),
                    tf.int64)
        ...
        def parse_blocks_from_record(records):
            ...
            Args:
                records: `tf.string` tensor of shape either `[]` or
                    `[batch_size]`.
            ...
            #    features is shaped
            #    [batch_size, len(flatten(self._data_spec))].
        ...
        def read_and_block_fixed_length_tfrecord_file(filename):
            ...
            Windowed records from filename come as a stream and prior to
            subsampling and shuffling, the stream contains blocks of the form:
        ...
        def read_and_block_variable_length_tfrecord_file(filename):
            """Read records from file, window them into variable len blocks."""
            def create_ta(spec):
            ...
            def parse_and_block_on_episode_boundaries(partial_tas, record):
                ...
                updated_tas = tf.nest.map_structure(
                        ...
                        partial_tas, frame)
                # If we see a LAST field, then emit empty TAs for the state and
                # updated TAs for the output.    Otherwise emit updated TAs for
                # the state and empty TAs for the output (the empty output TAs
                # will be filtered).
                return tf.cond(
            ...
            _func = lambda ta: ta.stack()  # NOQA
            stack_tas = lambda tas: tf.nest.map_structure(_func, tas)  # NOQA
            remove_intermediate_arrays = lambda tas: tas.step_type.size() > 0  # NOQA
        ...
        # Use tf.data.Dataset.from_tensors(0).map(...) to call the map() code
        # once per initialization.    This means that when the iterator is
        # reinitialized, we get a new list of files.
        ds = (tf.data.Dataset.from_tensors(0)
                ...
                .interleave(read_and_block_fn,
                            ...
                            block_length=(
                                self._data.per_file_shuffle_buffer_size),
                            num_parallel_calls=(num_parallel_calls or
                                                tf.data.experimental.AUTOTUNE))
                .shuffle(
                        buffer_size=interleave_shuffle_buffer_size,
                        seed=batch_seed))

        # Batch and parse the blocks. If `num_steps is None`, parsing has
        # already happened and we're not batching.
...
def _create_send_batch_py(rb_data):
    ...
    def send_batch_py(step_type, flat_serialized):
        ...
        for batch in range(batch_size):
            ...
            queue_buffer.append(
                    (rb_data.experiment_id, step_type[batch],
                     flat_serialized[batch]))
        ...
        if rb_data.batch_size is None:
            ...
            if batch_size > 64:
                logging.warning(
                    ...
                    'TFRecordReplayBuffer, which can cause python thread '
                    'contention and impact performance.')
...
def _process_write_queue(queue,  # NOQA
                         episodes_per_file,
                         time_steps_per_file,
...
def _encode_to_feature(t):
    """Encodes batched tensor `t` to a batched `tensorflow.Feature`.
    ...
    if t.dtype.is_integer or t.dtype.base_dtype == tf.bool:
        ...
        raise NotImplementedError(
            'Encoding tensor type %d is is not supported.' % t.dtype)
...
def _decode_feature(feature, spec, has_outer_dims):
    """Decodes batched serialized `tensorflow.Feature` to a batched `spec`.
    ...
    Returns:
        ...
        having type `spec.dtype`.    If `not has_outer_dims`, then the tensor
        has no `[batch_size, num_steps]` prefix.
    ...
    # This function assumes features come in as encoded tensorflow.Feature
    # strings with shape [batch, num_steps].
    ...
    if has_outer_dims:
        batch_size = (tf.compat.dimension_value(values.shape[0]) or
                      values_shape[0])
        num_steps = (tf.compat.dimension_value(values.shape[1]) or
                     values_shape[1])
        values = tf.reshape(
...
class WriterCleanup(object):
    ...
    def __del__(self):
        # NOTE(ebrevdo): This may spew a lot of noise at program shutdown due
        # to random module unloading.    If it does, it's safe to ignore but we
        # might consider wrapping it inside a broad try: ... except:, to avoid
        # the noise.
...
def _compression_type_string(record_options):
    if record_options is None:
        return None
    assert isinstance(record_options,
                      tf.io.TFRecordOptions), type(record_options)
